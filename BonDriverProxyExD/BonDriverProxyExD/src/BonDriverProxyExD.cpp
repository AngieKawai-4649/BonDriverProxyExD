				...
				while (1) {
					...
					if ((rtn = Separate(tstrLeft, _T(":"), tstrDllName, tstrDecoderNo))) {
						...
						if (itr == doubleCheck.end()) {
							...
							stDriver dri(tstrDllName.c_str());
...
//	int *p = new int[2];	// リーク検出テスト用
	...
	*phi = g_Host;
	...
	g_hWnd = CreateWindow(_T("bdpex"), _T("Information"), WS_OVERLAPPED | WS_SYSMENU | WS_THICKFRAME, CW_USEDEFAULT, 0, 800, 320, NULL, NULL, hInstance, NULL);
